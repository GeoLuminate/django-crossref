...
class CrossRefAuthorField(ModelMultipleChoiceField):
	...
	def _check_values(self, value):
		"""
		Given a list of author dicts (as returned by crossref), return a QuerySet of the corresponding objects. Values are queried on first, middle and family name. A new object will be created if not found in the database already.
		"""
		authors = []
		for author in value:
			# create_from = {k:v.strip().replace(',','') for k,v in author.items() if k in [f.name.replace('-','_') for f in self.queryset.model._meta.fields]}
			# defaults={}
